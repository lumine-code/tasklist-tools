...
module.exports = {
  ...
  eventHandler(e) {
    ...
    const editor = lumine.textEditors.getTextEditorForElement(e?.target, {
      includeMini: false,
    });
    const grammarSegments = editor?.getGrammar()?.scopeName?.split(".") ?? [];
    if (!grammarSegments.includes("tasklist")) return;
  ...
  deactivateStatusBar() {
    ...
    // ordered collection and inserts later tiles relative to it, so an
    // orphaned item left behind there breaks the next insertion.
